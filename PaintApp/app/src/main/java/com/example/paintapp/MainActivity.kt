...
suspend fun saveDrawingToGallery(context: Context, lines: List<Line>) {
    val bitmap = Bitmap.createBitmap(1800, 1920,Bitmap.Config.ARGB_8888)
    bitmap.applyCanvas {
    ...
    if (uri != null) {
        ...
        outputStream.use {
            if (it != null) {
                ...
                Toast.makeText(context, "Failed to Save Image in Gallery", Toast.LENGTH_SHORT).show()
            }
    ...
    Column(Modifier.fillMaxSize(), verticalArrangement = Arrangement.Top) {
        Row(Modifier.fillMaxWidth()
            ...
            Button(onClick = {isEraser = true}) {
                Text(text = "Eraser", fontSize = 6.sp)
            }
            Button(onClick = { lines.clear() }) {
                Text(text = "Reset", fontSize = 6.sp)
            }
            Button(onClick = {
